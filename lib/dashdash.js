...
Parser.prototype.parse = function parse(inputs) {
    ...
    outer: while (i < args.length) {
        ...
        // Long option
            ...
            // Check if we should negate the boolean value
            if (~arg.indexOf('--no-')) {
                negate = true;
                nameOffset = 5;
            }

            var name = arg.slice(nameOffset);
            var val = null;
            ...
            if (negate && ['bool','arrayOfBool'].indexOf(option.type) === -1) {
                throw new Error(format('cannot negate non-boolean option "--%s"', name));
            }
            ...
            if (!option) {
                ...
                if (!takesArg) {
                    addOpt(option, '--'+name, option.key, !negate, 'argv');
                } else if (val !== null) {
