...
var types = {
    bool: {
        takesArg: false,
        parseArg: function parseBool(option, optstr, arg) { return true; }
    },
    string: {
        takesArg: true,
        parseArg: function parseString(option, optstr, arg) {
    ...
    number: {
        takesArg: true,
        parseArg: function parseNumber(option, optstr, arg) {
            assert.string(arg, 'arg');
            var num = Number(arg);
            if (isNaN(num)) {
...
Parser.prototype.parse = function parse(argv) {
    ...
    function addOpt(option, optstr, name, val) {
        var parsedVal = types[option.type].parseArg(option, optstr, val);
        opts[name] = parsedVal;
        var item = {};
        item[name] = parsedVal;
        _order.push(item);
