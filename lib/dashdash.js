...
Parser.prototype.numArgsFromOption = function numArgsFromOption(option) {
...
Parser.prototype.parse = function parse(argv) {
    ...
    while (i < args.length) {
        ...
        // Long option
            ...
            var numArgs = this.numArgsFromOption(option);
            if (val !== null && numArgs !== 1) {
                if (numArgs === 0) {
                    throw new Error(format(
                        'argument given to "--%s" option that does not take one: "%s"',
                        name, arg));
                } else {
                    throw new Error(format('cannot use "--name=value" form '
                        + 'for option that takes multiple arguments: "%s"',
                        arg));
                }
            }
            //XXX parsing values for other types
            debug('numArgs:', numArgs, 'args.length:', args.length);
            if (numArgs === 0) {
                //XXX This is wrong for 'arrayOfBool'.
                opts[option.name] = true;
            } else if (val !== null) {
                opts[option.name] = val;
            } else if (i + numArgs >= args.length) {
                throw new Error(format('do not have enough args for "--%s" '
                    + 'option (takes %d args)', name, numArgs));
            } else if (numArgs === 1) {
                opts[option.name] = args[i + 1];
                i += numArgs;
            } else {
                opts[option.name] = args.slice(i + 1, i + 1 + numArgs);
                i += numArgs;
            }
        ...
        // Short option
            ...
            while (j < arg.length) {
                ...
                var numArgs = this.numArgsFromOption(option);
                debug('numArgs:', numArgs);
                if (numArgs === 0) {
                    //XXX This is wrong for 'arrayOfBool'.
                    opts[option.name] = true;
                } else if (val) {
                    //XXX option to disallow cuddled arg?
                    if (i + numArgs - 1 >= args.length) {
                        throw new Error(format('do not have enough args '
                            + 'for "-%s" option (takes %d args)',
                            name, numArgs));
                    }
                    if (numArgs === 1) {
                        opts[option.name] = val;
                    } else {
                        opts[option.name] = [val].concat(
                            args.slice(i + 1, i + numArgs));
                        i += numArgs - 1;
                    }
                    break;
                } else {
